  ...
  Future<void> _updateState(List<ConnectivityResult> result) async {
    switch (result.last) {
      ...
      case ConnectivityResult.none:
        ...
        throw NetworkException(TranslationsKeys.tkNoInternetMsg); // Trigger an error for no connection.
      default:
        connectionType.value = ConnectivityType.disconnected;
        _onLostConnection(); // Handle unknown state.
        throw NetworkException(TranslationsKeys.tkCheckNetworkFailedMsg); // Trigger a generic error.
    }
  }
  ...
  Future<void> _checkInternetConnection() async {
    if (!kIsWeb) {
      try {
        ...
        throw NetworkException(TranslationsKeys.tkNoInternetMsg); // Handle no internet scenario.
      }
    }
  }
